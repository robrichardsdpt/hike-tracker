...
const Ne67Patch = ({ mountainsTable }) => {
  ...
  const hike67Jsx = hike67.length
    ? `You have hiked ${hike67.length}/67 4000 foot mountains in New England!`
    : "Get started on your 67!";

  return (
    <div className="patch-card">
      <h2>NE 67 4k footers</h2>
      {hike67.length !== 67 && <ProgressChart hikes={hike67} totalPeaks={67} />}
      {hike67.length === 67 && (
        <div className="patch-image-container">
          <img src={NePatch} className="patch-image" alt="patch" />
        </div>
      )}
